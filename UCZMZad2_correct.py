...
df_public_sd = df_public[np.abs(df_public - df_public.mean()) <=
                         3*df_public.std()]
...
# x1
# x_test = df_public_q[['YearsCodePro']]
# reg.fit(x_test, y_true)

# x2
# x_test = df_public_q[['ConvertedComp']]
# reg.fit(x_test, y_true)

# y_test = pd.DataFrame(np.random.rand(df_len)*100)  # for x1 or x2

# x1 & x2
# x_test = df_public_q[['YearsCodePro', 'ConvertedComp']]
# reg.fit(x_test, y_true)
